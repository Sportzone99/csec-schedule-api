...
// M2M Client Credentials - should be in environment variables in production
...
// Token cache file
const TOKEN_CACHE_FILE = path.join(__dirname, '../../.nll-token-cache.json');
 ...
 * Caches the token to avoid excessive token requests
 */
async function getAccessToken() {
  ...
  if (fs.existsSync(TOKEN_CACHE_FILE)) {
    try {
      ...
      // Ignore cache errors, get new token
    }
  }
